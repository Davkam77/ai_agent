...
class RetrievalService:
    ...
    def retrieve(self, question: str, topic: str) -> tuple[list[RetrievedChunk], str | None, dict[str, object]]:
        bank_filter = self.classifier.detect_bank(question)
        ...
        if not selected_ranked:
            selected_ranked = [
                item
                for item in ranked
                if item[2] >= self.min_score and item[3] >= 0.05
            ]
        if not bank_filter:
            selected_ranked = self._diversify_by_bank(selected_ranked)
        ...
        debug = {
            ...
            "top_retrieved_chunks": [
                ...
                for chunk, combined, semantic, lexical in ranked[: self.top_k]
            ],
        ...
        logger.info(
            "Retrieval question=%r normalized_question=%r language=%s topic=%s bank=%s candidate_count=%s source_count=%s",
            question,
            normalized_question,
            detected_language,
            topic,
        ...
        logger.info("Retrieval top_sources=%s", debug["top_retrieved_sources"])
        logger.info("Retrieval top_chunks=%s", debug["top_retrieved_chunks"])
        if not selected:
    ...
    def _rank_candidates(
    ...
    ) -> list[tuple[RetrievedChunk, float, float, float]]:
        ...
        ranked: list[tuple[RetrievedChunk, float, float, float]] = []
    ...
    def _lexical_score(
        ...
        searchable_text = normalize_text(f"{chunk.bank_name}\n{chunk.page_title}\n{chunk.content}")
        ...
        product_phrase_match = any(
            ...
            for marker in (f"product {phrase}", f"product: {phrase}", f"branch {phrase}", f"branch: {phrase}")
        )
    ...
    def _diversify_by_bank(
        self,
        ranked: list[tuple[RetrievedChunk, float, float, float]],
    ) -> list[tuple[RetrievedChunk, float, float, float]]:
        if not ranked:
            return ranked
        first_pass: list[tuple[RetrievedChunk, float, float, float]] = []
        remainder: list[tuple[RetrievedChunk, float, float, float]] = []
        seen_banks: set[str] = set()
        for item in ranked:
            bank_name = item[0].bank_name
    ...
    @staticmethod
    def _build_source_debug(chunks: list[RetrievedChunk]) -> list[dict[str, str]]:
